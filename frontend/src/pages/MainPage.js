...
import DocumentGuideCard from '../components/eligibility/DocumentGuideCard';
...
export default function MainPage({ onError }) {
    ...
    // 분석 API에서 반환한 사이클 세션 ID 값.
    const [sessionId, setSessionId] = useState(null);

    const { fetchAnalyze, fetchAnalyzeText, fetchRetry, reset: resetApi, phase, errorMessage } = useAnalyze();

    // 자격 확인 훅(Eligibility check hook): O/X 질의응답 및 문서 안내 관리를 처리
    const {
        eligibilityStatus,
        currentQuestion,
        determination,
    ...
    }, [viewState, inputType, textInput, file, adminSummary, firstResponse, secondResponse, questionCount, showReferences, sessionId]);
    ...
    const handleSubmit = async () => {
        // 텍스트 모드: 텍스트 필요, 이미지 모드: 파일 필요
        if (inputType === 'text' && !textInput.trim()) {
            console.error('텍스트가 입력되지 않았습니다.');
            return;
        }
        ...
        try {
            const response = inputType === 'text'
                ? await fetchAnalyzeText(textInput)
                : await fetchAnalyze(file);
    ...
    const handleRetry = () => {
        ...
        setRetryLoading(false);
        setRetryError(false);
        // 앱을 재시작 시키고 초기화 할때 Eligibility 정보 초기화 플로우 처리
        resetEligibility();
        setSessionId(null);
        resetApi();
        sessionStorage.removeItem('appState');
    ...
    /**
     * 설명: 사용자가 해당 복지 프로그램에 대해 필요 서류 확인을 요청했을 때 작동합니다.
     * 작동 방식: eligibility 훅에 존재하는 fetchDocumentGuide 함수를 통해 안내사항과 식별 session id 호출.
     * 반환값: void
     */
    const handleRequestDocuments = async (programInfo, sid) => {
        await fetchDocumentGuide(programInfo, sid);
    };
    ...
    return (
        <Box sx={{
            ...
            <Container maxWidth="sm" sx={{ position: 'relative', zIndex: 10, pt: 4, px: 2 }}>
                ...
                {/* Eligibility Check Section - shown after references/2nd answer are visible */}
                {viewState === 'completed' && sessionId && (showReferences || questionCount >= 2 || retryError) && !retryLoading && (
                    <>
                        <Box sx={{ mt: 3 }} className="animate-fade-in-up">
                            <EligibilityCard
                                sessionId={sessionId}
                                programInfo={adminSummary}
